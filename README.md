...
The website has two equivalents under *From a listing*, and neither fetches anything from the dealer.
The simplest: save the listing page (Ctrl+S, "Webpage, HTML only") and drop the file on the sheet, or
paste the page source. The app reads the same DealerInspire analytics blob `scrape.py` reads out of the
text, on your device. The other is the **bookmarklet**: dragged to the bookmarks bar and clicked on a
vehicle page, it scans the page's inline scripts for that blob with the same marker and brace matcher
the CLI uses (it is not a window global on a live page, which is why an earlier version found only the
thin schema.org record), keeps the same subset of keys (`listing.payloadKeys` in the spec), and opens
the app with the record in the URL *fragment*. Fragments are never sent to a server, so the vehicle's
data goes from one of your tabs to another without touching lotstretcher.org.
`web/public/js/pipeline/listing.js` is a port of `normalize_vehicle()`, and
[`tests/test_listing_parity.py`](tests/test_listing_parity.py) runs both on one fixture under node and
compares every field, and holds the JS brace matcher to the Python one. The bookmarklet is generated by
the app you open it from, so a self-hosted install's bookmarklet opens that install.
